...
const MarkAttendance = () => {
  ...
  const handleCheckOut = async () => {
    ...
    const finalStatus = durationMillis / (1000 * 60 * 60) >= 8 ? 'P' : 'A';
    setStatus(finalStatus);
    const formattedDuration = formatDuration(durationMillis);
    setDuration(formattedDuration);
    await updateAttendanceInDB(checkInTime, now, finalStatus, formattedDuration);
    setIsLoading(false);
    alert(`Successfully checked out! Status: ${finalStatus}`);
};


  const updateAttendanceInDB = async (checkIn, checkOut, status, duration = '') => {
    ...
    const attendanceData = {
      ...
      status,
      duration, // Add this line to include duration in the attendance data
