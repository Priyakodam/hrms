...
function PerformanceMetricsList() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const trainingEvaluationRef = doc(collection(db, "trainingevaluation"));
      ...
      window.alert("Training Evaluation Saved Successfully.");
    ...
    const fetchTrainingEvaluation = async () => {
      try {
        const q = query(collection(db, "trainingevaluation"), orderBy("timestamp", "asc"));
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const evaluationRef = doc(db, "trainingevaluation", selectedEvaluation.id);
  ...
  const handleDelete = async (id) => {
    try {
      ...
      if (userConfirmed) {
        ...
        await deleteDoc(doc(db, "trainingevaluation", id));
  ...
  return (
    <div className="container">
      ...
      <h2 className="text-center mb-4">Training Evaluation</h2>
      ...
      <Modal show={showModal} onHide={toggleModal}>
        <Modal.Header closeButton>
          <Modal.Title>Add Training Evaluation</Modal.Title>
        </Modal.Header>
        <Modal.Body>
