  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const loanAdvanceRequestData = {
        ...
        status:'pending' // Add employee name to the request data
      };

      // Save the loan & advance request in the employee's collection
      const newRequestRef = await addDoc(collection(db, `managerloan_${loggedInEmployeeId}`), loanAdvanceRequestData);
