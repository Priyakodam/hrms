...
const fetchEmployeeData = async (loggedInEmployeeId) => {
  ...
  return Object.values(filteredEmployeeData);
};
...
const MonthlyReport = () => {
  ...
  const { data: employeeData = [], isError, isLoading, refetch } = useQuery(
      ...
      <form onSubmit={handleSubmit}>
        <div className='row'>
          <div className='col-md-4'>
            ...
            <input type="month" id="monthPicker" value={selectedMonth} onChange={handleMonthChange} className="mr-2" max={format(new Date(), 'yyyy-MM')}/>
          </div>
